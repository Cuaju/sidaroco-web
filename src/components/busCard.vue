<template>
  <article class="busCard" :class="{ expanded }" @click="toggleExpand">
    <div v-if="!expanded" class="collapsedRow">
      <span class="cell id">#{{ bus.id }}</span>
      <span class="cell name">{{ bus.name }}</span>
      <span class="cell">{{ bus.model }}</span>
      <span class="cell">{{ bus.plateNumber }}</span>
      <span class="cell statusBadge" :class="bus.status?.toLowerCase()">
        {{ bus.status }}
      </span>
      ...
      <div class="cardBody">
        <div class="header">
          <div class="titleRow">
            ...
            <span class="id">#{{ bus.id }}</span>
          </div>
          <div class="headerRight">
            <div class="statusBadge" :class="bus.status?.toLowerCase()">
              {{ bus.status }}
            </div>
...
.collapsedRow {
  display: grid;
  grid-template-columns: 60px 1.5fr 1fr 1fr 100px 40px;
  align-items: center;
  padding: 14px 20px;
  gap: 16px;
}
